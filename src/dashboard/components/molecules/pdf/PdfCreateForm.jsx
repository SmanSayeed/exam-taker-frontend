...
import { useCreatePdfMutation } from "@/features/pdfs/pdfApi";
import { Loader2 } from "lucide-react";
import { useState } from "react";
import { useForm, Controller } from "react-hook-form";
import { toast } from "sonner";

export default function PdfCreateForm() {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      await createPdf(formData).unwrap();
      toast.success("PDF and Image Uploaded Successfully!");
      reset();
      setFilePreview("");
      setImgPreview("");
      setPdfFile(null);
      ...
      console.error(err);
      toast.error(err?.data?.message || "An error occurred while uploading.");
    }
  };
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        />
        {filePreview && (
          <p className="text-sm mt-2 text-green-600">{filePreview}</p>
        )}
      </div>
